...
def submit_data():
    ...
    if setName == None or setName == '':
        ...
        try:
            Crappy_App_Logic.main(data, False)
        except Exception as err:
            print('An Error Occured: {0}'.format(err))
            error_window(err)
...
def wait_window():
    dialog = "Launch & quit application located on image, then click 'OK' button."
...
def wait_window_blank():
    dialog = "Download the application you want to the disk image created on the desktop."
    title = 'App Playpen Wait'
    try:
        if subprocess.check_call("""osascript -e 'display dialog "{0}" with title "{1}" ' """.format(dialog,title), shell=True) == 0:
            return True
...
def addPreScript():
    ...
    if(panel.runModal() == Cocoa.NSOKButton):
        ...
        n.views['preScript'].setStringValue_(str(path))
...
def addPostScript():
    ...
    if(panel.runModal() == Cocoa.NSOKButton):
        ...
        n.views['postScript'].setStringValue_(str(path))
...
def addCloseScript():
    ...
    if(panel.runModal() == Cocoa.NSOKButton):
        ...
        n.views['closeScript'].setStringValue_(str(path))
...
def blankBuild():
    ...
    postScript = n.views['postScript'].stringValue() or None
    closeScript = n.views['closeScript'].stringValue() or None
    collectedIdentifier = None
    collectedIcon = None
    collectedVersion = None
    ...
    if blankData.get('setname') is None:
        error_window('Please Enter an app name.')
    elif blankData.get('collectedsize') is None:
        error_window('Please Enter an image size (GB).')
    else:
        try:
            Crappy_App_Logic.main(blankData, True)
        except Exception as err:
            error_window(f"blank main failed: {err}")
...
def main():
    ...
    n.attach(blankBuild, 'blankBuildBtn')
